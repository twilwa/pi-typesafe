...
- Catalog-backed startup loading lives in `src/extension-catalog.ts`; `README.md` defines its local-only verification contract, and `docs/lane-manifests.md` covers the schema-valid worker manifests plus standalone catalog config. Keep catalog decisions in the selection receipt and never download an artifact.
