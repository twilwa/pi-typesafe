...
- The Pi entry is `src/extension.ts`; question policy lives in `src/checks.ts`, failure/timeout handling in `src/sidecar.ts`, and runtime response checks in `src/verdict.ts`. Keep shadow mode the default and judge failures a quiet no-op.
